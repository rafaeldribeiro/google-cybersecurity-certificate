...
*   **Este evento poderia ser:** 
    Classificado como um ataque **DoS direto**. A planilha de monitoramento confirma que todo o tráfego está saindo de uma única máquina, diferenciando de um DDoS, que usaria múltiplos computadores espalhados pela internet para camuflar a ação.
...
Quando o agente mal intencionado dispara milhares de pacotes **SYN** ao mesmo tempo a partir do IP `203.0.113.0`, ele quebra esse fluxo de propósito. O atacante faz o passo 1 **SYN**, o servidor executa o passo 2 **SYN, ACK** e gasta memória guardando o espaço daquela conexão. Porém, o atacante ignora o passo 3 e **nunca envia o pacote ACK de confirmação**. 
...
*   A partir do **item de log nº 125**, a capacidade do servidor zera, ele para de responder completamente a qualquer tráfego legítimo, deixando a equipe de publicidade impossibilitada de trabalhar e gerando prejuízos comerciais para a agência.
